...
get_netdevice_map () {
    ...
    do {
        ...
        res  = GetAdaptersAddresses (
            AF_INET, GAA_FLAG_SKIP_DNS_SERVER, NULL, adapters.data (), &size);
    ...
    do {
        ...
        do {
            ...
            auto& addr_in =
                *reinterpret_cast<SOCKADDR_IN*> (ip->Address.lpSockaddr);
            auto& addr = addr_in.sin_addr;
            boost::asio::ip::address_v4::bytes_type bytes;
            assert (sizeof (addr) == sizeof (bytes));
            std::memcpy (&bytes, &addr, sizeof (bytes));
            ret.insert (NetworkInterfaceMap::value_type (
                name, boost::asio::ip::address_v4 (bytes)));
        } while (ip = ip->Next);
