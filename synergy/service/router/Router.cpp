...
Router::add_peer (tcp::endpoint endpoint, bool const immediate) {
    ...
    asio::spawn (acceptor_.get_io_service (), [this, endpoint, immediate](auto ctx) {
        ...
        for (; attempt <= kConnectRetryLimit; ++attempt) {
            ...
            if (ec) {
                routerLog ()->error ("Outbound connection {} to {} failed to complete SSL "
                                     "handshake, aborting: {}",
                                     connection->id (), endpoint, ec.message());
                break;
            }
      ...
      ssl_context_(boost::asio::ssl::context::tlsv12)
{
    if (!::SSL_CTX_set_cipher_list (ssl_context_.native_handle(),
        ...
        throw std::runtime_error ("Failed to select a secure TLS cipher for P2P networking.");
    }
...
{
    return R"CERT(-----BEGIN CERTIFICATE-----
...
Router::sslKey()
{
    return R"PKEY(-----BEGIN PRIVATE KEY-----
...
Router::sslDH()
{
    return R"DHPARAMS(-----BEGIN DH PARAMETERS-----
