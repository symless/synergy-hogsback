...
public:
    ...
    ClientProxy& proxy() const { return proxy_; }
...
public:
    explicit ClientProxyConnection (tcp::socket socket, int32_t client_id, std::string screen_name) :
        client_id_ (client_id), socket_ (std::move(socket)), screen_name_(screen_name) {
    }
...
ClientProxy::ClientProxy (asio::io_service& io, Router &router, int port): io_(io), port_(port), router_(router) {
...
void
ClientProxy::
connect (int32_t client_id, const std::string& screen_name) {
    /* Connect loop */
    asio::spawn (io_, [this, client_id, screen_name](auto ctx) {
        ...
        socket.async_connect (tcp::endpoint
            (ip::address_v4::from_string("127.0.0.1"), port_), ctx[ec]);
        ...
        if (ec) {
            ...
            connections_.emplace_back
                (std::make_shared<ClientProxyConnection>(std::move(socket), client_id, std::move(screen_name)));
            connections_.back()->start(*this);
        }
    });
}

Router &ClientProxy::router() const
{
    return router_;
}
...
void
ClientProxyMessageHandler::operator()(Message const& message, int32_t source) const
{
...
void ClientProxyMessageHandler::handle(const CoreMessage& cm, int32_t source) const
{
    auto& connections = proxy().connections_;
    auto it = std::find_if (begin(connections), end(connections), [source](auto& connection) {
        return connection->client_id_ == source;
